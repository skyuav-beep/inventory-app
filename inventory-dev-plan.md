...
- API: `DATABASE_URL`, `JWT_SECRET`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `ALERT_COOLDOWN_MINUTES`, `QUIET_HOURS=22-07`
...
- [ ] React 레이아웃(사이드바·상단바), 테이블/카드/모달
- [ ] 제품/입고/출고/반품 화면 + 모바일 풀스크린 모달
- [ ] 대시보드(부족품목 배너 + 차트)
- [ ] 텔레그램 설정 UI + 테스트 발송
- [ ] Excel/CSV 업로드(비동기 Job) + 결과 리포트 UI
- [ ] 알림 발송 워커(쿨다운·심야·로그)

### 스프린트 3 — 하드닝 & 배포

- [ ] 권한 세분화(리소스별 읽기/쓰기) UI/가드 적용
- [ ] 감사/오류 로깅, 헬스체크, 레이트 리밋
- [ ] 도커/리버스 프록시/nginx 설정, .env 체계화
